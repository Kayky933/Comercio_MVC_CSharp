...
namespace Mercado.MVC.Validation.ValidateModels
{
    public class ProdutoValidation : AbstractValidator<ProdutoModel>
    {
        public ProdutoValidation(bool edicao = false)
        {
            ...
            RuleFor(x => x.Preco_Unidade).NotEmpty().WithMessage(ProdutoErrorMessages.PrecoNulo)
            ...
            RuleFor(x => x.Unidade_De_Medida).NotEmpty().WithMessage(ProdutoErrorMessages.UnidadeMedidaNula)
            ...
            if (!edicao)
                RuleFor(x => x.Quantidade_Produto).NotEmpty().WithMessage(ProdutoErrorMessages.QuantidadeProdNula)
